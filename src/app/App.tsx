      ...
      <AnimatePresence>
        ...
        {viewingProduct && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-md p-2">
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full max-h-[70vh] max-w-4xl flex flex-col md:flex-row relative bg-white shadow-[6px_6px_0_rgba(0,0,0,1)] border-4 border-black overflow-hidden">
              ...
              <div className="w-full md:w-1/2 bg-gray-50 flex items-center justify-center relative border-b-4 md:border-b-0 md:border-r-4 border-black">
                 <img src={viewingProduct.image} className="w-full h-full object-contain max-h-[30vh] md:max-h-[60vh] p-2" alt={viewingProduct.name} />
              </div>

              {/* Details Side */}
              <div className="w-full md:w-1/2 bg-white p-4 md:p-6 overflow-y-auto flex flex-col justify-center brutal-scrollbar">
        ...
        {isCheckoutOpen && checkoutItems.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-md p-2">
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full max-h-[75vh] max-w-4xl flex flex-col md:flex-row relative bg-white shadow-[6px_6px_0_rgba(0,0,0,1)] border-4 border-black overflow-hidden">
              ...
              {/* Order Summary Side */}
              <div className="w-full md:w-5/12 bg-gray-50 p-4 md:p-6 overflow-y-auto border-b-4 md:border-b-0 md:border-r-4 border-black brutal-scrollbar">
                 <h3 className="text-lg font-orbitron font-bold mb-4 uppercase tracking-widest text-black border-b-4 border-black pb-2">Order Summary</h3>
                 
                 <div className="space-y-2 mb-4">
                    {checkoutItems.map((item, idx) => (
                       <div key={idx} className="flex items-center gap-3">
                          <img src={item.image} className="w-12 h-16 object-cover bg-gray-200 border border-black" alt={item.name} />
                          <div>
                 ...
                 <div className="border-t-4 border-black pt-3 space-y-1">
                    <div className="flex justify-between items-center text-[9px] font-orbitron font-bold text-gray-600 uppercase tracking-widest">
                       <span>Subtotal</span>
                    ...
                    <div className="flex justify-between items-center pt-2 text-sm font-orbitron font-bold uppercase tracking-widest text-black">
                       <span>Total</span>
              ...
              {/* Checkout Form Side */}
              <div className="w-full md:w-7/12 p-4 md:p-6 overflow-y-auto brutal-scrollbar">
                 <div className="w-full">
                   <h3 className="text-lg font-orbitron font-bold mb-4 uppercase tracking-widest text-black border-b-4 border-black pb-2">Secure Checkout</h3>

                   <div className="bg-black text-[var(--theme-cyan)] p-2 mb-4 border-2 border-black">
                     <p className="font-orbitron text-[9px] font-bold uppercase tracking-widest mb-1">
                       <AlertTriangle size={10} className="inline mr-1 -mt-1" /> Philippines Only
                     </p>
                     <p className="font-jakarta text-[9px] text-gray-300 leading-relaxed">
                   ...
                   <form className="space-y-3" onSubmit={(e) => { 
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                       <div>
                         <label className="block text-[9px] font-orbitron font-bold uppercase mb-1 text-black tracking-widest">Full Name</label>
                         <input type="text" required className="w-full border-2 border-black p-1.5 font-jakarta bg-gray-50 text-black focus:border-[var(--theme-accent)] outline-none transition-colors text-xs" />
                       </div>
                       <div>
                         <label className="block text-[9px] font-orbitron font-bold uppercase mb-1 text-black tracking-widest">Email</label>
                         <input type="email" required className="w-full border-2 border-black p-1.5 font-jakarta bg-gray-50 text-black focus:border-[var(--theme-accent)] outline-none transition-colors text-xs" />
                       </div>
                     </div>
                     
                     <div>
                       <label className="block text-[9px] font-orbitron font-bold uppercase mb-1 text-black tracking-widest">Shipping Address</label>
                       <input type="text" required placeholder="Street, City, Province, Zip" className="w-full border-2 border-black p-1.5 font-jakarta bg-gray-50 text-black focus:border-[var(--theme-accent)] outline-none transition-colors text-xs" />
                     </div>

                     {/* Payment Toggle */}
                     <div>
                        <label className="block text-[9px] font-orbitron font-bold uppercase mb-1 text-black tracking-widest">Payment Method</label>
                        <div className="flex gap-2">
                           <button type="button" onClick={() => setPaymentMethod("GCash")} className={`flex-1 py-2 border-2 font-orbitron font-bold text-[9px] uppercase tracking-widest transition-all ${paymentMethod === "GCash" ? "bg-black text-[var(--theme-cyan)] border-black shadow-[2px_2px_0_var(--theme-accent)]" : "bg-white text-black border-black hover:bg-gray-100"}`}>GCash</button>
                           <button type="button" onClick={() => setPaymentMethod("Bank")} className={`flex-1 py-2 border-2 font-orbitron font-bold text-[9px] uppercase tracking-widest transition-all ${paymentMethod === "Bank" ? "bg-black text-[var(--theme-cyan)] border-black shadow-[2px_2px_0_var(--theme-accent)]" : "bg-white text-black border-black hover:bg-gray-100"}`}>Bank Transfer</button>
                        </div>
                     </div>

                     <div className="bg-gray-100 p-3 border-2 border-dashed border-black">
                       <p className="text-[9px] font-orbitron font-bold uppercase text-black mb-1 tracking-widest">Transfer Details</p>
                       {paymentMethod === "GCash" ? (
                          <div className="font-jakarta text-[10px] text-gray-800 space-y-0.5">
                     ...
                     <div>
                       <label className="block text-[9px] font-orbitron font-bold uppercase mb-1 text-black tracking-widest">Upload Receipt</label>
                       <input type="file" required className="w-full border-2 border-black p-1 font-jakarta text-[10px] bg-gray-50 text-black file:bg-black file:text-white file:border-0 file:px-3 file:py-1 file:font-orbitron file:uppercase file:text-[9px] file:tracking-widest file:cursor-pointer hover:file:bg-[var(--theme-accent)] transition-all cursor-pointer" />
                     </div>
                     
                     <button type="submit" className="w-full bg-[var(--theme-accent)] text-white font-orbitron font-bold py-3 border-4 border-black hover:bg-black hover:text-white transition-colors uppercase mt-4 text-xs tracking-widest shadow-[4px_4px_0_var(--theme-cyan)]">
                       Complete Order
                     </button>
                   </form>
                 </div>
